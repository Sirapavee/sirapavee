...
// const SpiralFlyByPlanetScene: FC<Props> = ({ onReady }) => {
...
//           >
//             {[
//               { title: 'Project Alpha', tag: 'React / Three.js', c: '#5a6b8c' },
//               { title: 'Project Beta', tag: 'Next.js / GSAP', c: '#6b7b55' },
//               { title: 'Project Gamma', tag: 'WebGL / Shaders', c: '#7b5a6b' },
//             ].map((p, i) => (
//               <div
//                 key={i}
...
//                     borderRadius: 8,
//                     marginBottom: 16,
//                     background: `linear-gradient(135deg,${p.c}22,${p.c}08)`,
...
//         </section>
//       </div>
//       <style>{`@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@300;400;600&display=swap');
//         *{margin:0;padding:0;box-sizing:border-box}html{scroll-behavior:auto}body{background:#020212}
//         ::selection{background:rgba(100,140,200,0.25);color:#fff}
//         ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-track{background:transparent}
//         ::-webkit-scrollbar-thumb{background:rgba(100,140,200,0.15);border-radius:2px}
//         .project-card:hover{background:rgba(255,255,255,0.04)!important;border-color:rgba(255,255,255,0.12)!important;transform:translateY(-4px)}`}</style>
//     </div>
//   );
// };

// export default SpiralFlyByPlanetScene;
...
interface ResponsiveConfig {
  holdXScale: number;
  holdYOffset: number;
  // Mobile: [minCoverage, maxCoverage] — biggest planet → min%, smallest → max%
  // Desktop: null — use native radius × desktopScale
  mobileCoverage: [number, number] | null;
  desktopScale: number; // flat scale for lg+
  trajScale: number;
  cameraZ: number;
  fov: number;
  panelMode: 'side' | 'top';
...
// Planet radius range for coverage mapping
const PLANET_R_MIN = 1.4;
const PLANET_R_MAX = 3.5;

// Compute display scale for a planet given responsive config
function getDisplayScale(planetRadius: number, resp: ResponsiveConfig): number {
  if (!resp.mobileCoverage) return resp.desktopScale;

  const [maxCov, minCov] = resp.mobileCoverage; // bigger planet → less coverage
  // Viewport height at planet distance
  const dist = resp.cameraZ - 2; // planet holdPos.z is ~2
  const vpHeight = 2 * dist * Math.tan(((resp.fov / 2) * Math.PI) / 180);
  // Map planet radius to coverage: small→maxCov, large→minCov
  const t = clamp((planetRadius - PLANET_R_MIN) / (PLANET_R_MAX - PLANET_R_MIN), 0, 1);
  const coverage = lerp(maxCov, minCov, t);
  const targetRadius = (vpHeight * coverage) / 2;
  return targetRadius / planetRadius;
}

const RESP: Record<Breakpoint, ResponsiveConfig> = {
  // sm/md: planet diameter is 85–95% of viewport, then holdYOffset clips bottom 25%
  // Result: visible planet fills ~60–70% of screen height
  // [smallPlanetCoverage, bigPlanetCoverage] — small planets get bigger coverage so they fill similarly
  'sm': {
    holdXScale: 0,
    holdYOffset: -3.0,
    mobileCoverage: [0.95, 0.85],
    desktopScale: 1,
    trajScale: 0.4,
    cameraZ: 10,
    fov: 65,
  ...
  // lg+: native planet sizes, desktop layout
  'lg': {
    holdXScale: 0.8,
    holdYOffset: 0,
    mobileCoverage: null,
...
const PLANETS: PlanetDef[] = [
  {
    ...
    subtitle: 'Living World · Biosphere Peak',
    radius: 3.0,
    holdPos: new THREE.Vector3(5, 0.5, 2),
    entryZ: -50,
    entryAngle: Math.PI * 0.3,
    exitDir: new THREE.Vector3(-15, -10, 50),
    trajectoryType: 'logarithmic',
    hasRing: false,
    ringInner: 0,
    ringOuter: 0,
    ...
    colors: {
      base: [0.02, 0.08, 0.28],
      band1: [0.05, 0.18, 0.38],
      band2: [0.12, 0.35, 0.1],
      accent: [0.9, 0.94, 0.98],
      atmo: [0.18, 0.45, 0.85],
    },
    experience: {
      role: 'Senior Frontend Engineer',
  ...
  },
  {
    ...
    subtitle: 'Gas Giant · Crimson Tempest',
    radius: 3.5,
    holdPos: new THREE.Vector3(-5, 0, 3),
    entryZ: -45,
    entryAngle: 0,
    exitDir: new THREE.Vector3(16, 8, 50),
    trajectoryType: 'corkscrew',
    hasRing: true,
    ringInner: 4.5,
    ringOuter: 7.5,
  ...
  },
  {
    ...
    subtitle: 'Rocky Volcanic · Active Geology',
    radius: 2.2,
    holdPos: new THREE.Vector3(4, 1, 4),
    entryZ: -55,
    entryAngle: Math.PI,
    exitDir: new THREE.Vector3(-12, -12, 50),
    trajectoryType: 'banking',
    ...
    colors: {
      base: [0.12, 0.1, 0.1],
      band1: [0.28, 0.24, 0.22],
      band2: [0.18, 0.15, 0.14],
      accent: [1.0, 0.35, 0.02],
      atmo: [0.5, 0.15, 0.05],
    },
    experience: {
      role: 'Creative Developer',
  ...
  },
  {
    ...
    subtitle: 'Precambrian Rock · Dawn Era',
    radius: 1.4,
    holdPos: new THREE.Vector3(-4, 0.5, 2),
    entryZ: -35,
    entryAngle: 0,
    exitDir: new THREE.Vector3(-18, 6, 50),
    trajectoryType: 'helix',
    hasRing: false,
    ringInner: 0,
    ringOuter: 0,
...
const MainPlanet: FC<{
  ...
  useFrame(({ clock }) => {
    ...
    if (groupRef.current) {
      ...
      if (visible) {
        groupRef.current.position.copy(pos);
        groupRef.current.scale.setScalar(scale * getDisplayScale(def.radius, r));
        groupRef.current.rotation.y = clock.getElapsedTime() * 0.04;
      }
    }
  });
...
// Camera position only — no lookAt, no rotation override.
// Camera keeps its default forward orientation (-Z).
// Other components can freely set camera rotation without conflict.
const CameraRig: FC<{ camRef: MutableRefObject<CamTarget> }> = ({ camRef }) => {
  const { camera } = useThree();
  useFrame(() => {
    const c = camRef.current;
...
// ─── CAMERA LOGIC — position only, no rotation override ───
function computeCamera(gp: number, resp: ResponsiveConfig): CamTarget {
  const camZ = resp.cameraZ;
  const base: CamTarget = { x: 0, y: 1, z: camZ };
  ...
  for (let i = 0; i < PLANETS.length; i++) {
    ...
    if (lp > 0 && lp < 1) {
      ...
      // ENTER: camera drifts toward incoming planet
      if (lp <= 0.35) {
        const ease = easeOutCubic(lp / 0.35);
        base.x = pos.x * xTrack * ease;
        base.y = 1 + pos.y * yTrack * ease;
        base.z = camZ + (pos.z - camZ) * 0.08 * ease;
      }
      // HOLD: gentle offset toward planet
      else if (lp <= 0.65) {
        base.x = pos.x * xTrack * 0.8;
        base.y = 1 + pos.y * yTrack * 0.8;
        base.z = camZ;
      }
      // EXIT: ease back to neutral
      else {
        const ease = 1 - easeInCubic((lp - 0.65) / 0.35);
        base.x = pos.x * xTrack * ease * 0.5;
        base.y = 1 + pos.y * yTrack * ease * 0.5;
        base.z = camZ + (pos.z - camZ) * 0.05 * (1 - ease);
      }
  ...
  // Dwarf swarm: centered, gentle vertical drift
  if (gp >= 0.8) {
    const dp = clamp((gp - 0.8) / 0.2, 0, 1);
    base.x = 0;
    base.y = 1 + Math.sin(dp * Math.PI) * 0.8;
...
const SpiralPlanets: FC<Props> = ({ onReady }) => {
  const progressRef = useRef(0);
  const respRef = useResponsiveRef();
  const bp = useBreakpoint();
  const panelMode = RESP[bp].panelMode;
  const camRef = useRef<CamTarget>({ x: 0, y: 1, z: RESP[bp].cameraZ });
  ...
  const triggerEntrance = useCallback(() => {
    ...
    tl.to(
      camRef.current,
      { x: 0, y: 1, z: r.cameraZ, duration: 1.5, ease: 'power3.out' },
      0,
    );
  }, [phase, initScroll, respRef]);
